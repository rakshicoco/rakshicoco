...
import { Plus, Search, HandCoins, Calendar, CreditCard } from 'lucide-react'
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-100">Farm Payments (Outward)</h1>
      ...
      <div className="flex items-center space-x-2">
        <div className="relative w-full sm:max-w-sm">
          ...
          <Input 
            type="search" 
            placeholder="Search by Payment ID, Farm..." 
      ...
      {!hasPayments ? (
        <EmptyState
          icon={HandCoins}
          title="No farm payments yet"
          description="Record payments or advances issued to farmers against coconut harvest procurements."
          actionHref="/dashboard/farm-payments/new"
            ...
            {payments!.map((payment: any) => (
              ...
              >
                <div className="flex items-start justify-between gap-2 border-b border-slate-100 dark:border-slate-800/60 pb-2.5">
                  <div>
                    ...
                    <p className="text-sm font-medium text-slate-900 dark:text-slate-100 mt-0.5">
                      {payment.farms?.owner_name || 'Direct Farmer'}
                      {payment.farms?.village ? ` (${payment.farms.village})` : ''}
                    </p>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  ...
                  {payment.purchase_id && (
                    <div className="col-span-2 text-xs text-slate-500 pt-1 border-t border-slate-100 dark:border-slate-800">
                      Purchase: <Link href={`/dashboard/purchases/${payment.purchase_id}`} className="text-primary font-mono hover:underline">{payment.purchase_id}</Link>
                    </div>
                  )}
                </div>
              </div>
