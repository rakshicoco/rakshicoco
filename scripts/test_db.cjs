...
async function main() {
  console.log('1. Testing read from farms...');
  const { data: initialFarms, error: readErr } = await client.from('farms').select('*');
  console.log('Initial farms count:', initialFarms ? initialFarms.length : 0, 'Read error:', readErr);

  console.log('2. Testing insert into farms...');
  ...
  console.log('Updated farms count:', updatedFarms ? updatedFarms.length : 0);

  // Test insert into purchases if farm exists
  if (inserted && inserted.id) {
    console.log('3. Testing insert into purchases...');
